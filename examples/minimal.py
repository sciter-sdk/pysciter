"""Minimal PySciter sample for Windows."""
...
def main():
    ...
    sc_callback = SciterHostCallback(on_sciter_callback)
    ok = sapi.SciterSetCallback(hWnd, sc_callback, c_void_p(0))

    try:
        url = u"examples/minimal.htm"
        ok = sapi.SciterLoadFile(hWnd, url)
    except:
        etype, evalue, estack = sys.exc_info()
        pass
